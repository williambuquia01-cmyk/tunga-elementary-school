...
/* PHASE_24_PPSSH_EDITABLE — the framework moved from a hardcoded constant to admin-editable
   server-stored data. PPSSH_DEFAULT is the seed used on first load and the "Reset to default"
   target. The active framework lives in useStore("ppssh_v1") inside PPSSHPage. */
const PPSSH_DEFAULT=[
...
{ /* PHASE_4_1_FIX_BC_V2 - inject DrawingML check inside box using boxIdx */
              if (boxIdx !== null && boxIdx !== undefined) {
                const _yOffsets = [0, 187325, 374650, 561975, 749300, 937259, 1124584, 1311909, 1499869, 1687194, 1874519, 2061844, 2249169];
                const _y = _yOffsets[boxIdx];
                if (_y !== undefined) {
  ...
  if (matchedType && checkboxAnchors[matchedType]) {
    ...
    if (firstIdx >= 0) {
      const runStart = xml.lastIndexOf("<w:r>", firstIdx) >= 0
        ...
        : xml.lastIndexOf("<w:r ", firstIdx);
      if (runStart >= 0) {
        /* PHASE_4_1_FIX_BC_V2: leave-type check injection disabled, DrawingML used instead */
      }
    }
  }
...
export default function App(){
  ...
  /* PHASE_24_PPSSH_EDITABLE — the 6-Domain framework itself is now admin-editable. Stored under
     ppssh_v1 (NOT scoped by school year — same framework applies across years). Falls back to
     PPSSH_DEFAULT on first load if the server has no override. */
  const[ppssh,setPpssh]=useStore("ppssh_v1",PPSSH_DEFAULT);
...
<script>window.onload=function(){setTimeout(function(){window.print();},500);}<\/script></body></html>`;
  ...
  /* ═══ PPSSH (Phase 24 — admin-editable framework) ═══ */
  const PPSSHPage=()=>{
    ...
    /* PHASE_24_PPSSH_EDITOR — admin operations on the framework itself.
       Domain ops: add/edit name+color/delete (with confirm).
       Strand ops: add/edit code+label/delete (with confirm).
       Reset: wipe server override, restore PPSSH_DEFAULT. */
    const editDomainName=(did)=>{const d=ppssh.find(x=>x.id===did);if(!d)return;const newName=prompt("New Domain name:",d.name);if(newName===null)return;const trimmed=newName.trim();if(!trimmed)return;setPpssh(prev=>prev.map(x=>x.id===did?{...x,name:trimmed}:x));};
    const editDomainColor=(did)=>{const d=ppssh.find(x=>x.id===did);if(!d)return;const newColor=prompt("New color (hex like #1B4D7E):",d.color);if(newColor===null)return;const trimmed=newColor.trim();if(!/^#[0-9A-Fa-f]{6}$/.test(trimmed)){alert("Invalid hex color. Use format like #1B4D7E");return;}setPpssh(prev=>prev.map(x=>x.id===did?{...x,color:trimmed}:x));};
    const deleteDomain=(did)=>{if(!confirm(`Delete this entire Domain and all its strands? Existing uploaded MOVs for this domain will become orphaned but not deleted from storage.`))return;setPpssh(prev=>prev.filter(x=>x.id!==did));setPpssSel(null);};
    const addDomain=()=>{const name=prompt("Name of new Domain:");if(!name||!name.trim())return;const nextId=Math.max(0,...ppssh.map(d=>d.id))+1;setPpssh(prev=>[...prev,{id:nextId,name:name.trim(),color:"#1B4D7E",strands:[]}]);};

    const editStrand=(did,sCode)=>{const d=ppssh.find(x=>x.id===did);const s=d?.strands.find(x=>x.code===sCode);if(!s)return;const newCode=prompt(`Strand code (e.g. "${sCode}"):`,s.code);if(newCode===null)return;const newLabel=prompt("Strand description:",s.label);if(newLabel===null)return;const codeT=newCode.trim();const labelT=newLabel.trim();if(!codeT||!labelT)return;setPpssh(prev=>prev.map(x=>x.id===did?{...x,strands:x.strands.map(st=>st.code===sCode?{code:codeT,label:labelT}:st)}:x));};
    const deleteStrand=(did,sCode)=>{if(!confirm(`Delete strand ${sCode}? Uploaded MOVs for this strand will become orphaned but not deleted from storage.`))return;setPpssh(prev=>prev.map(x=>x.id===did?{...x,strands:x.strands.filter(st=>st.code!==sCode)}:x));};
    const addStrand=(did)=>{const d=ppssh.find(x=>x.id===did);if(!d)return;const code=prompt(`Strand code (e.g. "${d.id}.${d.strands.length+1}"):`,`${d.id}.${d.strands.length+1}`);if(!code||!code.trim())return;const label=prompt("Strand description:");if(!label||!label.trim())return;setPpssh(prev=>prev.map(x=>x.id===did?{...x,strands:[...x.strands,{code:code.trim(),label:label.trim()}]}:x));};

    const resetPPSSH=()=>{if(!confirm("Reset the entire PPSSH framework back to the original 6-Domain default? This cannot be undone (but uploaded MOVs are preserved)."))return;setPpssh(PPSSH_DEFAULT);};

    return(<><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
      <h2 style={{fontSize:20,fontWeight:700}}>PPSSH MOVs — {ppssh.length} Domain{ppssh.length===1?"":"s"}</h2>
      {isAdmin&&!dom&&<div style={{display:"flex",gap:6}}>
        <Btn sm outline color="#1B4D7E" onClick={addDomain}>{IC.plus} Add Domain</Btn>
        <Btn sm outline color="#c0392b" onClick={resetPPSSH}>🔄 Reset</Btn></div>}</div>
      {!dom?<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))",gap:10}}>
        {ppssh.map(d=>{const tot=Object.keys(ppsMov).filter(k=>k.startsWith(`${d.id}-`)).reduce((a,k)=>a+ppsMov[k].length,0);
          return<div key={d.id} style={{background:"#fff",borderRadius:14,padding:16,borderLeft:`5px solid ${d.color}`,transition:"box-shadow .15s",position:"relative"}}>
            <div onClick={()=>setPpssSel(d.id)} style={{cursor:"pointer"}}>
              ...
              <div style={{fontSize:11,color:"#999",marginTop:4}}>{d.strands.length} strands · <strong>{tot} MOVs</strong></div></div>
            {isAdmin&&<div style={{display:"flex",gap:4,marginTop:8,paddingTop:8,borderTop:"1px dashed #eee"}}>
              <button onClick={(e)=>{e.stopPropagation();editDomainName(d.id);}} style={{flex:1,padding:"4px 6px",border:`1px solid ${d.color}`,background:"transparent",color:d.color,borderRadius:5,cursor:"pointer",fontSize:11,fontFamily:"inherit"}} title="Edit name">✏️ Name</button>
              <button onClick={(e)=>{e.stopPropagation();editDomainColor(d.id);}} style={{flex:1,padding:"4px 6px",border:`1px solid ${d.color}`,background:"transparent",color:d.color,borderRadius:5,cursor:"pointer",fontSize:11,fontFamily:"inherit"}} title="Edit color">🎨 Color</button>
              <button onClick={(e)=>{e.stopPropagation();deleteDomain(d.id);}} style={{padding:"4px 8px",border:"1px solid #c0392b",background:"transparent",color:"#c0392b",borderRadius:5,cursor:"pointer",fontSize:11,fontFamily:"inherit"}} title="Delete domain">🗑️</button></div>}
          </div>;})}</div>
      :(<><button onClick={()=>setPpssSel(null)} style={{background:"none",border:"none",cursor:"pointer",fontSize:13,color:"#666",display:"flex",alignItems:"center",gap:4,marginBottom:10,fontFamily:"inherit"}}>{IC.back} All Domains</button>
        <div style={{background:`linear-gradient(135deg,${dom.color},${dom.color}cc)`,borderRadius:14,padding:"16px 20px",color:"#fff",marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
          <div><div style={{fontSize:11,opacity:.7}}>Domain {dom.id}</div><h3 style={{margin:"2px 0",fontSize:17,fontWeight:700,textTransform:"capitalize"}}>{dom.name}</h3>
            <div style={{fontSize:12,opacity:.65}}>{dom.strands.length} strands</div></div>
          {isAdmin&&<Btn sm outline color="#fff" onClick={()=>addStrand(dom.id)}>{IC.plus} Strand</Btn>}</div>
          ...
          return<div key={strand.code} style={{background:"#fff",borderRadius:10,padding:"12px 14px",marginBottom:8}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
              ...
              <div style={{display:"flex",gap:4,flexShrink:0}}>
                {isAdmin&&<>
                  <button onClick={()=>editStrand(dom.id,strand.code)} style={{width:28,height:28,borderRadius:6,background:"transparent",color:dom.color,border:`1px solid ${dom.color}`,cursor:"pointer",fontSize:12}} title="Edit strand">✏️</button>
                  <button onClick={()=>deleteStrand(dom.id,strand.code)} style={{width:28,height:28,borderRadius:6,background:"transparent",color:"#c0392b",border:"1px solid #c0392b",cursor:"pointer",fontSize:12}} title="Delete strand">🗑️</button>
                </>}
